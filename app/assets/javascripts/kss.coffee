    ...
    try
      for stylesheet in document.styleSheets
        ...
        for rule, idx in stylesheet.cssRules
          if rule.type is CSSRule.STYLE_RULE and (hover.test(rule.selectorText) or disabled.test(rule.selectorText) or active.test(rule.selectorText) or visited.test(rule.selectorText))
